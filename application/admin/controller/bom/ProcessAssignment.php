<?php

namespace app\admin\controller\bom;

use app\common\controller\Backend;
use app\admin\model\ProcessAssignment as ProcessAssignmentModel;
use app\admin\model\Component as ComponentModel;
use app\admin\model\MaterialUsage as MaterialUsageModel;
use app\admin\model\Process as ProcessModel;
use think\Db;
use think\exception\DbException;
use think\exception\PDOException;
use think\exception\ValidateException;

/**
 * 工艺分配管理
 */
class ProcessAssignment extends Backend
{
    /**
     * ProcessAssignment模型对象
     * @var \app\admin\model\ProcessAssignment
     */
    protected $model = null;

    public function _initialize()
    {
        parent::_initialize();
        $this->model = new ProcessAssignmentModel;
        
        // 获取部件列表
        $componentModel = new ComponentModel;
        $componentList = $componentModel->with('product')->select();
        $componentOptions = [];
        foreach ($componentList as $component) {
            $componentOptions[$component->id] = $component->product->name . ' > ' . $component->name;
        }
        $this->view->assign("componentList", $componentOptions);
        
        // 获取材料使用列表
        $materialUsageModel = new MaterialUsageModel;
        $materialUsageList = $materialUsageModel->with(['component.product', 'rawMaterial'])->select();
        $materialUsageOptions = [];
        foreach ($materialUsageList as $usage) {
            $materialUsageOptions[$usage->id] = $usage->component->product->name . ' > ' . 
                $usage->component->name . ' > ' . $usage->rawMaterial->name;
        }
        $this->view->assign("materialUsageList", $materialUsageOptions);
        
        // 获取工艺列表
        $processModel = new ProcessModel;
        $processList = $processModel->column('name', 'id');
        $this->view->assign("processList", $processList);
        
        // 获取工艺分配列表（用于父工艺选择）
        $assignmentList = $this->model->column('alias', 'id');
        $this->view->assign("assignmentList", $assignmentList);
    }

    /**
     * 查看
     */
    public function index()
    {
        //当前是否为关联查询
        $this->relationSearch = true;
        //设置过滤方法
        $this->request->filter(['strip_tags', 'trim']);
        if ($this->request->isAjax()) {
            //如果发送的来源是Selectpage，则转发到Selectpage
            if ($this->request->request('keyField')) {
                return $this->selectpage();
            }
            list($where, $sort, $order, $offset, $limit) = $this->buildparams();

            $list = $this->model
                    ->with(['component.product', 'materialUsage.rawMaterial', 'process', 'parent'])
                    ->where($where)
                    ->order($sort, $order)
                    ->paginate($limit);

            foreach ($list as $row) {
                $row->visible(['id','component_id','material_usage_id','parent_assignment_id','process_id','alias','sequence','cost_override','is_affected_by_imposition','is_transformative','createtime','updatetime']);
                $row->visible(['component', 'materialUsage', 'process', 'parent']);
                
                if ($row->component) {
                    $row->getRelation('component')->visible(['name', 'product']);
                    if ($row->component->product) {
                        $row->getRelation('component')->getRelation('product')->visible(['name']);
                    }
                }
                
                if ($row->materialUsage) {
                    $row->getRelation('materialUsage')->visible(['alias', 'rawMaterial']);
                    if ($row->materialUsage->rawMaterial) {
                        $row->getRelation('materialUsage')->getRelation('rawMaterial')->visible(['name']);
                    }
                }
                
                if ($row->process) {
                    $row->getRelation('process')->visible(['name', 'cost_type']);
                }
                
                if ($row->parent) {
                    $row->getRelation('parent')->visible(['alias']);
                }
            }

            $result = array("total" => $list->total(), "rows" => $list->items());

            return json($result);
        }
        return $this->view->fetch('bom/process_assignment/index');
    }

    /**
     * 添加
     */
    public function add()
    {
        if ($this->request->isPost()) {
            $params = $this->request->post("row/a");
            if ($params) {
                $params = $this->preExcludeFields($params);

                if ($this->dataLimit && $this->dataLimitFieldAutoFill) {
                    $params[$this->dataLimitField] = $this->auth->id;
                }
                $result = false;
                Db::startTrans();
                try {
                    //是否采用模型验证
                    if ($this->modelValidate) {
                        $name = str_replace("\\model\\", "\\validate\\", get_class($this->model));
                        $validate = is_bool($this->modelValidate) ? ($this->modelSceneValidate ? $name . '.add' : $name) : $this->modelValidate;
                        $this->model->validateFailException(true)->validate($validate);
                    }
                    $result = $this->model->allowField(true)->save($params);
                    Db::commit();
                } catch (ValidateException $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                } catch (PDOException $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                } catch (Exception $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                }
                if ($result !== false) {
                    $this->success();
                } else {
                    $this->error(__('No rows were inserted'));
                }
            }
            $this->error(__('Parameter %s can not be empty', ''));
        }
        return $this->view->fetch('bom/process_assignment/add');
    }

    /**
     * 编辑
     */
    public function edit($ids = null)
    {
        $row = $this->model->get($ids);
        if (!$row) {
            $this->error(__('No Results were found'));
        }
        $adminIds = $this->getDataLimitAdminIds();
        if (is_array($adminIds) && !in_array($row[$this->dataLimitField], $adminIds)) {
            $this->error(__('You have no permission'));
        }
        if ($this->request->isPost()) {
            $params = $this->request->post("row/a");
            if ($params) {
                $params = $this->preExcludeFields($params);
                $result = false;
                Db::startTrans();
                try {
                    //是否采用模型验证
                    if ($this->modelValidate) {
                        $name = str_replace("\\model\\", "\\validate\\", get_class($this->model));
                        $validate = is_bool($this->modelValidate) ? ($this->modelSceneValidate ? $name . '.edit' : $name) : $this->modelValidate;
                        $row->validateFailException(true)->validate($validate);
                    }
                    $result = $row->allowField(true)->save($params);
                    Db::commit();
                } catch (ValidateException $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                } catch (PDOException $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                } catch (Exception $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                }
                if ($result !== false) {
                    $this->success();
                } else {
                    $this->error(__('No rows were updated'));
                }
            }
            $this->error(__('Parameter %s can not be empty', ''));
        }
        $this->view->assign("row", $row);
        return $this->view->fetch('bom/process_assignment/edit');
    }

    /**
     * 根据部件ID或材料使用ID获取工艺分配列表
     */
    public function getByTarget()
    {
        if (!$this->request->isAjax()) {
            $this->error('非法请求');
        }

        $componentId = $this->request->get('component_id');
        $materialUsageId = $this->request->get('material_usage_id');
        
        if (!$componentId && !$materialUsageId) {
            $this->error('部件ID或材料使用ID不能同时为空');
        }

        try {
            $query = $this->model->with(['process']);
            
            if ($componentId) {
                $query->where('component_id', $componentId);
            }
            if ($materialUsageId) {
                $query->where('material_usage_id', $materialUsageId);
            }

            $assignments = $query->order('sequence asc')->select();

            $result = [];
            foreach ($assignments as $assignment) {
                $result[] = [
                    'id' => $assignment->id,
                    'alias' => $assignment->alias,
                    'process_name' => $assignment->process->name ?? 'Unknown',
                    'cost_override' => $assignment->cost_override,
                    'is_affected_by_imposition' => $assignment->is_affected_by_imposition,
                    'is_transformative' => $assignment->is_transformative,
                    'sequence' => $assignment->sequence
                ];
            }

            $this->success('获取成功', '', $result);
        } catch (\Exception $e) {
            $this->error('获取失败: ' . $e->getMessage());
        }
    }

    /**
     * 批量更新序号
     */
    public function updateSequence()
    {
        if (!$this->request->isPost()) {
            $this->error('非法请求');
        }

        $sequences = $this->request->post('sequences/a');
        if (!$sequences) {
            $this->error('序号数据不能为空');
        }

        Db::startTrans();
        try {
            foreach ($sequences as $id => $sequence) {
                $this->model->where('id', $id)->update(['sequence' => $sequence]);
            }
            Db::commit();
            $this->success('更新成功');
        } catch (\Exception $e) {
            Db::rollback();
            $this->error('更新失败: ' . $e->getMessage());
        }
    }

    /**
     * 复制工艺分配
     */
    public function copy($ids = null)
    {
        $row = $this->model->get($ids);
        if (!$row) {
            $this->error(__('No Results were found'));
        }

        if ($this->request->isPost()) {
            $params = $this->request->post("row/a");
            if ($params) {
                $params = $this->preExcludeFields($params);
                
                // 复制数据，但清除主键和时间戳
                $copyData = $row->toArray();
                unset($copyData['id'], $copyData['createtime'], $copyData['updatetime']);
                
                // 合并新提交的数据
                $copyData = array_merge($copyData, $params);
                
                $result = false;
                Db::startTrans();
                try {
                    $newAssignment = new ProcessAssignmentModel;
                    $result = $newAssignment->allowField(true)->save($copyData);
                    Db::commit();
                } catch (ValidateException $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                } catch (PDOException $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                } catch (Exception $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
                }
                if ($result !== false) {
                    $this->success();
                } else {
                    $this->error(__('No rows were inserted'));
                }
            }
            $this->error(__('Parameter %s can not be empty', ''));
        }
        
        // 预设复制的工艺分配别名
        $row->alias = $row->alias . '_副本';
        $this->view->assign("row", $row);
        return $this->view->fetch('bom/process_assignment/add');
    }
}